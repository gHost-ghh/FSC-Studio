...
- Native Runtime now matches the Python title/group layout, creates complete ONNX session groups asynchronously, reports the actual CPU/DirectML/CUDA/QNN provider, and runs integrity check, backup, WAL checkpoint, VACUUM, and legacy conversion on worker connections. Legacy progress is queued back to the UI without `processEvents` re-entry.
...
- Dense Mesh uses the exact 478-point MediaPipe tessellation in native Points or Textured modes. Textured mode uses depth-tested image sampling, darkens only back-facing triangle surfaces, supports drag/zoom/reset, and overlays the cached 68-point landmarks on the dense surface only when enabled. Generation runs the official landmark network directly through ONNX Runtime, anchored to the selected face bbox and five keypoints so multi-face photos cannot silently cache the largest different face. Invalid old caches are rejected and can be repaired with `fsc_native_probe <db> repair-invalid-meshes <face_landmarks_detector.onnx>`.
- Native Inno Setup installers package Qt, OpenCV, ONNX Runtime, the selected DirectML/CUDA/QNN provider, Dense Mesh ONNX, and InsightFace models without Python or user data. x64 packaging runs a Windows-platform launch smoke; formal release signing remains a distribution task, not a Python-parity gap.
