...
- Library now includes text/person/tag/review/quality filters, Selected and Batch metadata tabs for person assignment, tags, review state, ignored state, and notes, plus an Activity tab with import progress and processed-image preview; native SQLite reads/writes `face_tags`. Multi-image and recursive-folder imports run on a background worker, batch SQLite writes in groups of 50, and coalesce preview rendering at 50 ms so the visual activity does not throttle inference.
